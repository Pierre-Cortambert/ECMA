...
function Dual(n :: Int64, s :: Int64, t :: Int64,  S :: Int64,  d1 :: Int64, d2 :: Int64, p :: Vector{Int64}, ph :: Vector{Int64}, A :: Array{Int64}, d :: Array{Int64}, D :: Array{Float64})
	...
	@constraint(m, b0*d2 + 2 * sum(b[k] for k in 1:n) + sum(p[i]*sum(x[i,j]*A[i,j] for j in 1:n) for i in 1:n) )

	@constraint(m, [i in 1:n ; i!=t ], b0 + b[i] >= ph[i]*sum(x[i,k]*A[i,k] for k in 1:n))
